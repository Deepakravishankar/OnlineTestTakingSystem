...
function check(inputTag){
	...
	case "address":
		...
		pattern=new RegExp("^[A-Za-z0-9-, '.()\/]{0,100}$");
		if(!pattern.test(inputTag.value)){
	...
	case "school": case "gender": 
		if(inputTag.children[0].selected){
			document.getElementById(inputTag.id+"-error").innerHTML="Please select an option";
			return false;
		}
		else
			document.getElementById(inputTag.id+"-error").innerHTML="";
...
function validate(form){
	...
	result&=check(document.getElementById("gender"));	
	result&=check(document.getElementById("school"));
