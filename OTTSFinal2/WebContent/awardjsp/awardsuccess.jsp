<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1" import="com.ilp.otts.bean.*"%>
...
{
	var x=document.getElementById('awardid');	

	if (!x.value.match(/^[a-zA-Z0-9]+$/))
	{
		//alert("Please Enter a Valid Award ID");
	document.getElementById('awardid').focus;
	//return false;
	}
	...
	var x=document.getElementById('awardname');	
...
var y=document.getElementById('awardamount');	

	if (!y.value.match(/^[0-9]/))
	{
	...
	var z=document.getElementById("awardreason");	
...
<h4 style="font-family:gentiumbasic;color:maroon;font-size:30px">Update Award Details</h4>

<% 
		ArrayList<Bean> viewresult = (ArrayList<Bean>)(request.getAttribute("awardlist"));
			
            
            
           /* for(int i=0; i<viewresult.size(); i++) {
            	AwardBean view = viewresult.get(i);
            	
            	System.out.println("Test ID: " + view.getAwardid());
            	System.out.println("Student ID: " +view.getStudentid());
            	System.out.println("Score: " + view.getScore());
            	System.out.println("Percentage: " + view.getAverage());
            	System.out.println();
            }*/
%>
            
     
         <form name="update" method="post" action="<%=request.getContextPath()%>/Controller" onsubmit="return validateForm()">
        <center> <table border=1 style="position:relative;left:-80px">
 <% 
 for(int i=0; i<viewresult.size(); i++) {
	Bean view = viewresult.get(i);
 %>
         
         <tr>
         <td>
         Award id:</td><td><input type="text" name="awardid"  value="<%=  view.getAwardid() %>"></input>
         </td></tr>
        <tr> <td>
          Award type:</td><td>  <input type="text" name="awardtype"  value="<%=  view.getAwardtype() %>"></input><br></br>
         </td></tr>
         <tr><td>
        Award Name:</td><td> <input type="text" name="awardname" value="<%=  view.getAwardname() %>"></input><br></br>
         </td>
         </tr>
         <tr><td>
        Amount: </td><td><input type="text" name="awardamount" value="<%=  view.getAwardamount() %>"></input><br></br>
         </td>
         </tr>
         <tr><td>
        Award Reason: </td><td><input type="text" name="awardreason" value="<%=  view.getAwardreason() %>"></input><br></br>
         </td>
         </tr>
         <tr>
         <td></td>
        <td><br><input type="submit" name="page" value="update"></input></td></tr>
     <% 
     } 
   %>
         </table>
