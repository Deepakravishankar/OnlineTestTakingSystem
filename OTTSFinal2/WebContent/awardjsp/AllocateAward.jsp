<%@ page language="java" import="java.util.*,com.ilp.otts.bean.*" contentType="text/html; charset=ISO-8859-1"
...
function validateForm()
{
	...
	{
		alert("Please select the award to be allocated");
...
<h4 style="font-family:gentiumbasic;color:maroon;font-size:30px;position:relative;left:-1px">Allocate Award</h4>


<form name="form" action="<%=request.getContextPath()%>/Controller" method="post" onsubmit="return validateForm()">
<input type="hidden" name="page" value="get-eligible-students">
Select award:
<select name="awardid" id="id">
<option>select</option>
<% ArrayList<Bean> list=(ArrayList<Bean>)request.getAttribute("awardlist");
	if(list!=null){
		if(!list.isEmpty()){
			for(Bean ab: list){
%>				
				<option seleted="awardid" id=<%=ab.getAwardid()%> value=<%=ab.getAwardid()%>>
				<%=ab.getAwardname()+" "+ab.getAwardamount() %>
				</option>			
<%			}
...
<form name="myform" action="<%=request.getContextPath()%>/Controller" method="post" onsubmit="return validate()">
<input type="hidden" name="page" value="allocate-awards">
<table border="1" style="position:relative;left:-180px;top:-80px">
<% ArrayList<Bean> rblist=(ArrayList<Bean>)request.getAttribute("eligible-students"); 
if(rblist!=null){
	if(!rblist.isEmpty()){
%>

		Eligible Students for <%=request.getAttribute("awardname")%>
		<tr></tr><tr>
		<td>Select</td>
		<td>TestID</td>
		<td>StudentID</td>
...
<tr>
<td><input type="checkbox" name="id" value="<%=rb.getTestid()%>,<%=rb.getStudentid()%>,<%=request.getAttribute("awardid")+","+request.getAttribute("awardname")%>"></input></td>

<td><%=rb.getTestid()%></td>
<td><%=rb.getStudentid()%></td>
<td><%=rb.getSchoolid()%></td>
</tr>

<%
		}
	}else{
		out.println("No Eligible Students found");
	}
...
<input type="submit" value="Allocate"></input>
</form>
