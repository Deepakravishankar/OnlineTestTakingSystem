<%@ page language="java" import="java.util.ArrayList, com.ilp.otts.bean.TestTeacherBean"contentType="text/html; charset=ISO-8859-1"
...
ttb=(ArrayList<TestTeacherBean>)request.getAttribute("allteacher");
t2=(ArrayList<String>)request.getAttribute("selectedteacher");
%>
<form name="form" action="/OTTS/TestController" method="post" onsubmit="return validate();">
<table border="2">
<tr><td>Available teachers:</td><td>
<%if(ttb!=null){for(int i=0;i<ttb.size();i++){
	if(t2.contains(ttb.get(i).getTeacherid())){

	%>

<input type="checkbox" name="teacher" value="<%=ttb.get(i).getTeacherid()  %> " checked><%=ttb.get(i).getTeacherName() %>
<%} else{%>
<input type="checkbox" name="teacher" value="<%=ttb.get(i).getTeacherid() %>"><%=ttb.get(i).getTeacherName() %>
<%}}} %>
</td></tr>
</table>
<input type="hidden" name="page" value="updatetestteacherfinal"></input>
