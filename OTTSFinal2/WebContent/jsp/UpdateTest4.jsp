...
<h1>Select test to be updated</h1>
<% ArrayList<TestBean> t=new ArrayList<TestBean>();
t=(ArrayList<TestBean>)session.getAttribute("viewupdatetestid");
%>
<form name="form" action="/OTTS/TestController" method="post" onsubmit="return validateForm();">
<label>Test name:</label>
<%if(!t.isEmpty()){for(int i=0;i<t.size();i++){ %>
<input type="radio" name="testid" value="<%=t.get(i).getTestid() %>"><%=t.get(i).getTestName() %>
<%}} %>
<input type="hidden" name="page" value="updatetestid"></input>
