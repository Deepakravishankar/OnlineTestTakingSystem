        ...
        <div class="span2">
          <div class="well sidebar-nav">
            <ul class="nav nav-list">
				<li><a class="active" href="/OTTS/WelcomeJSP/<%=session.getAttribute("role")%>.jsp" >Home</a></li>          
            </ul>
          </div><!--/.well -->
        </div><!--/span-->
...
<div class="span6">
	...
	<%
		ArrayList<AdminBean> adminList = (ArrayList<AdminBean>) request.getAttribute("result");
	...
	<%
		if (adminList != null) {
			...
			if (!adminList.isEmpty()) {
				AdminBean adminBean=adminList.get(0);
				//out.println("inside if 2 loop");
	%>
	<form action="/OTTS/ManageAdminServlet" method="post" onsubmit="return validate(this)">
		<input type="hidden" name="type" value="update-profile"> <input
			type="hidden" name="id" value=<%=adminBean.getId() %>>
		<table>
			<tr>
				<td>FirstName:</td>
				<td><input type="text" name="firstname"
					value=<%=adminBean.getFirstname()%> disabled="true"></input></td>
			</tr>
			<tr>
				<td>MiddleName:(optional)</td>
				<td><input type="text" name="middlename"
					value=<%=adminBean.getMiddlename()%> disabled="true"></td>
			</tr>
			<tr>
				<td>LastName:</td>
				<td><input type="text" name="lastname"
					value=<%=adminBean.getLastname()%> disabled="true"></td>
			</tr>
			<tr>
				<td>DOB:</td>
				<td><input type="text" name="dob" value=<%=adminBean.getDob()%> disabled="true">
				</td>
			</tr>
			<tr>
				<td>Email:</td>
				<td><input type="text" name="email"
					value=<%=adminBean.getEmail()%> id="email" onblur="check(this)"></td>
					<td id="email-error" class="error"></td>
			</tr>
			<tr>
				<td>PhoneNumber:</td>
				<td><input type="text" name="phonenumber"
					value=<%=adminBean.getPhonenumber()%> id="phonenumber" onblur="check(this)"></td>
					<td id="phonenumber-error" class="error"></td>
			</tr>
			<tr>
				<td>Address:</td>
				<td><input type="text" name="address"
					value=<%=adminBean.getAddress()%> disabled="true"></td>
			</tr>

			<tr align="center">
				<td colspan="2"><input class="btn btn-success" type="submit" name="submit"
					value="Update Details"></td>
			</tr>

		</table>
	</form>
	<form action="/OTTS/ManageAdminServlet" method="post">
	<input type="hidden" name="type" value="update-profile-secretquestion">
		<table>
			<tr>
				<td>Secret Question:</td><td> <input type="text" name="secretquestion"> </td>
			</tr>
			<tr>
				<td>Answer:</td><td> <input type="text" name="secretanswer"> </td>
			</tr>
			<tr><td></td><td> <input class="btn btn-info" type="submit" value="Set Question & Answer" style="width:100px"> </td></tr>
		</table>
	</form>
	<%
		} else {
